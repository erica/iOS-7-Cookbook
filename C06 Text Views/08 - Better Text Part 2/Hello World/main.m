...
- (void)loadAccessoryView
{
    ...
    // A bit of a hack to deal with a bug in layout in the toolbar (only effects B & I)
    UIBarButtonItem * boldButton = BARBUTTON_TARGET(self, @"B", @selector(toggleBoldface:));
    [boldButton setTitleTextAttributes:@{ NSBaselineOffsetAttributeName : @-6 } forState:UIControlStateNormal];
    [items addObject:boldButton];
    
    UIBarButtonItem * italicsButton = BARBUTTON_TARGET(textView, @"I", @selector(toggleItalics:));
    [italicsButton setTitleTextAttributes:@{ NSBaselineOffsetAttributeName : @-6 } forState:UIControlStateNormal];
    [items addObject:italicsButton];
...
// A bit of a hack to deal with the textview not resizing the selection when the text changes size due to toggling bold
